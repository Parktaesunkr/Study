...
public class Train042602 {
	...
	// int형 비트 구성
		...
		// x의 pos 위치에 있는 비트로부터 n개의 비트를 1로 변경한 값을 반환
		static int setN(int x, int pos, int n) {
			return x | (~(~0 << n) << pos);
		}
		// x의 pos 위치에 있는 비트로부터 n개의 비트를 0으로 변경한 값을 반환
		static int resetN(int x, int pos, int n) {
			return x & ~(~(~0 << n) << pos);
		}
		
		
		// x의 pos 위치에 있는 비트로부터 n개의 비트를 반전시킨 값을 반환
	...
	public static void main(String[] args) {
		...
		System.out.println("정수 x의 pos번째 비트로부터 n개 비트를 변경합니다.");
